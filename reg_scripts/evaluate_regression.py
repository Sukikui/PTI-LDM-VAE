...
def save_metrics(output_dir: Path, metrics: dict[str, float], args: argparse.Namespace, files: list[str]) -> None:
    ...
    payload = {"metrics": metrics, "args": args_payload, "files": files}
...
def normalize_configs(config: dict[str, Any], args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    Returns:
        tuple[dict[str, Any], dict[str, Any], dict[str, Any]]: Data, training, and evaluation configuration blocks.
    """
    ...
    return data_cfg, train_cfg, eval_cfg
...
def main() -> None:
    ...
    data_cfg, train_cfg, eval_cfg = normalize_configs(config, args)
